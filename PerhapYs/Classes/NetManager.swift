...
struct NetManagerHelper{
    
    struct BaseParam : Encodable{
    ...
    //    var sign : String
       ...
       static func languageString() -> String{
            ...
            if let languages = UserDefaults.standard.object(forKey: "AppleLanguages") as? Array<Any> , let local = languages.first as? String{
                if local == "en-CN"{  //中文:zh-Hans-CN
                    return "en_us"
                }
            }
            return "zh_cn"
        ...
        static func make() -> BaseParam{
            let uuid = AppInfo().identifier
            let client = "ios"
            let version = AppInfo().app_version
            let timestamp = String(Int(Date.getTimesstamp()))
            let language = self.languageString()
            
            let param = BaseParam.init(uuid: uuid, client: client, version: version, timestamp: timestamp, language: language)
            
            return param
        }
    }
    
    static func jointBaseParam<T:Encodable>(param:T?) -> [String:String]?{
        
        let baseParam = BaseParam.make()
        var baseDic = toDictionary(with: baseParam)!
        
        if param != nil , let dic = toDictionary(with: param){
            baseDic.merge(dic) { one, two in
                return one
            }
        }
        return self.jointSign(para: self.change(dic: baseDic))
    }
    
    private static func jointSign(para:[String:String]) -> [String:String]{
        
        if let timestamp = para["timestamp"] , let intTime = timestamp.toInt{
         
            let isSingle = intTime % 2 == 1
            var sorted : [Dictionary<String,String>.Element] = []
            if isSingle{
                sorted = para.sorted(by: {$0 < $1})
            ...
            for data in sorted {
                ...
                signTemp += keyValueStr
            }
            
            let SignKey = "OelvRaICnoE7Q2fQ7aFE9XJVg4UPmTVj"
            signTemp += "salt=\(SignKey)"
            let sign = signTemp.md5String
            
            var finalParam : [String:String] = [:]
            finalParam.merge(para) { one, two in
                return one
            }
            finalParam["sign"] = sign
            return finalParam
        }
        
        return [:]
    }
    private static func change(dic:Dictionary<String,Any>) -> [String:String]{
        
        var new : [String : String] = [:]
        for key in dic.keys {
            let value = dic[key]
            if let valueInt = value as? Int{
                let stringValue = String(valueInt)
                new[key] = stringValue
            }
            else if let valueString = value as? String{
                let stringValue = String(valueString)
                new[key] = stringValue
            }
        }
        return new
